...
public class UIChatInput : MonoBehaviour
{
	...
	public delegate void OnPrivateChatPopup();

	public static UIChatInput current;
	...
	public GameObject _privateChatEventReceiver;

	public string _privateChatFunctionName = "OnPrivateChatPopup";

	public OnPrivateChatPopup onPrivateChatPopup;
	...
	private void OnSelect(bool isSelected)
	{
		...
		if (isSelected)
		{
			...
			if (Application.platform == RuntimePlatform.IPhonePlayer || Application.platform == RuntimePlatform.Android)
			{
				...
				{
					mKeyboard = TouchScreenKeyboard.Open(mText, TouchScreenKeyboardType.ASCIICapable, autoCorrect, false, false, true, Localization.instance.Get("liaotianshi_desc2"));
				}
				UIMessageDispatch.Instance.PostMessage(EUIMessageID.UISquare_OpenMobileChat, null, null);
			}
			else
			{
	...
	private void Update()
	{
		...
		if (mKeyboard.done)
		{
			if (!mKeyboard.wasCanceled && !TouchScreenKeyboard.hideInput && !UIDataBufferCenter.Instance._bManualCloseMobileChat)
			{
			...
			else
			{
				...
				eventReceiver.SendMessage(functionName, string.Empty, SendMessageOptions.DontRequireReceiver);
				current = null;
				selected = false;
			}
			UIDataBufferCenter.Instance._bManualCloseMobileChat = false;
			UIMessageDispatch.Instance.PostMessage(EUIMessageID.UISquare_CloseMobileChat, null, null);
		}
		else if (!mKeyboard.active || mKeyboard.wasCanceled)
		{
			mKeyboard = null;
			current = this;
			if (onSubmit != null)
