...
public class UI_Pallet_L_Btn : TUIButton
{
	...
	private float PointY2L(float y)
	{
		float num = y - base.transform.position.y + size.y / 2f;
		return num / size.y;
	}

	private float PointX2L(float x)
	{
	...
	public override bool HandleInput(TUIInput input)
	{
		...
		if (input.inputType == TUIInputType.Began)
		{
			if (PtInControl(input.position))
			{
				...
				PostEvent(this, 1, PointX2L(input.position.x), PointY2L(input.position.y), null);
				return true;
			}
			return false;
		}
		if (input.fingerId == m_iFingerId)
		{
			if (input.inputType == TUIInputType.Moved)
			{
				if (PtInControl(input.position))
				{
					...
					PostEvent(this, 4, PointX2L(input.position.x), PointY2L(input.position.y), null);
				}
				else if (m_bPressed)
				{
					m_bPressed = false;
			...
			else if (input.inputType == TUIInputType.Ended)
			{
				...
				{
					Show();
					PostEvent(this, 2, PointX2L(input.position.x), PointY2L(input.position.y), null);
					PostEvent(this, 3, PointX2L(input.position.x), PointY2L(input.position.y), null);
				}
				else
				{
					Show();
