...
public class UIChatPopupList : MonoBehaviour
{
	...
	public static UIChatPopupList current;
		...
		set
		{
			...
			if (mSelectedItem != value)
			{
				...
				if (textLabel != null)
				{
					...
					Debug.Log("textLabel.text=" + textLabel.text);
				}
				flag = true;
			}
	...
	private void OnPrivateChatPopup()
	{
		RefreshChatFriendsContainer();
	}

	private void RefreshChatFriendsContainer()
	{
		Debug.Log("2222222222222222");
		_chatFriendsContainer.gameObject.transform.parent.gameObject.SetActive(true);
		int count = items.Count;
		_chatFriendsContainer.InitContainer(NGUI_COMUI.UI_Container.EBoxSelType.Single);
		_chatFriendsContainer.InitBoxs(count, true);
		for (int i = 0; i < count; i++)
		{
			NGUI_COMUI.UI_BoxData data = new UISquare_ChatFriendBoxData(items[i]);
			_chatFriendsContainer.SetBoxData(i, data);
		}
		if (count > 6)
		{
			_chatFriendsContainer.SetMoveForce(new Vector3(0f, 1f, 0f));
		}
		else
		{
			_chatFriendsContainer.SetMoveForce(Vector3.zero);
		}
	}
}
