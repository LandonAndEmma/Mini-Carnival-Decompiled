...
public class iServerConfigData : MonoBehaviour
{
	...
	protected string m_sUrl = "http://account.trinitigame.com/game/CoMDH";
	...
	private void Update()
	{
		if (m_ltExpireConfig.Count > 0)
		{
			...
			if (cExpireConfig.m_State == CExpireConfig.State.Wait)
			{
				...
				iServerFile.Instance.Visit(m_sUrl + "/CoMDH_ClientConfig/" + cExpireConfig.m_sFileName + ".txt", OnFetchConfig_S, OnFetchConfig_F);
			}
		}
	}
	...
	public void Fetch(OnEvent onsunccess, OnEvent onfailed)
	{
		...
		m_OnFailed = onfailed;
		iServerFile.Instance.Visit(m_sUrl + "/" + m_sUrl_MD5 + ".txt", OnFetchMD5_S, OnFetchMD5_F);
	}
	...
	{
		Debug.Log("OnFetchMD5_S " + sFileData);
		...
		try
		{
			if (cExpireConfig.m_State == CExpireConfig.State.Fetch)
			{
				...
				if (configInfo != null && configInfo.m_UnPack != null)
				{
					...
					Debug.Log("fetch success " + cExpireConfig.m_sFileName);
				}
	...
	protected string TransformXML2TXT(string srcpath, string dstpath, string key)
	{
		...
		{
			string text2 = XXTEAUtils.Encrypt(text, key);
