...
public class UIButtonPlayAnimation : MonoBehaviour
{
	...
	private void Play(bool forward)
	{
		...
		// Compute the direction explicitly instead of doing arithmetic on the enum.
		Direction direction;
		if (forward)
		{
		...
		{
			// pick the opposite direction explicitly
			direction = (playDirection == Direction.Forward) ? Direction.Reverse : Direction.Forward;
		}
