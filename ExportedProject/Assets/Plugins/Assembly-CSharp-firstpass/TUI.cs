...
public class TUI : TUIInputHandleModel
{
	...
	{
		Vector3 position = new Vector3(input.position.x, input.position.y, tuiCamera.camera.nearClipPlane);
		Vector3 vector = tuiCamera.camera.ScreenToWorldPoint(position);
