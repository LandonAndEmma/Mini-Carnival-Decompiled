...
public class TUISliderEx : TUIControlImpl
{
    ...
    // Added explicit 2-argument overload to avoid optional parameter issues on older Mono compilers
    public void Set(float input, TUIScaleThumb rect)
    {
        Set(input, rect, false);
    }

    // Main implementation (removed default parameter to be explicit)
    public void Set(float input, TUIScaleThumb rect, bool postEvent)
    {
    ...
    // Added explicit 1-argument overload to avoid optional parameter issues on older Mono compilers
    ...
    // Main implementation (removed default parameter to be explicit)
    private void Set(float input, bool postEvent)
    {
        float num = (rawValue = Mathf.Clamp01(input));
        if (mStepValue == num)
        {
