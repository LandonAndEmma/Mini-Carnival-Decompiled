...
{
	public delegate void PageChanged(int nCurPage);
	...
	public event PageChanged ProcePageChanged;
	...
	{
		for (int num = pages.Count; num != 0; num--)
		{
			Remove(0, destroy);
		}
		pages.Clear();
	...
	public bool Forward(float time = 0f)
	{
		...
		{
			currentPageIndex++;
			if (this.ProcePageChanged != null)
			{
				this.ProcePageChanged(currentPageIndex);
			}
			foreach (TUIPageEx page in pages)
			{
