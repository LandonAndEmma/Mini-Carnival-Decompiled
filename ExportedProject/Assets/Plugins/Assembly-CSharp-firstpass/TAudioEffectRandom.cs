...
public class TAudioEffectRandom : ITAudioEvent
{
	...
	private void Awake()
	{
		...
		m_volumBase = base.audio.volume;
		m_pitchBase = base.audio.pitch;
	...
	private void Trigger(bool delay)
	{
		...
		base.audio.volume = Mathf.Clamp01(Random.Range(m_volumBase - volumOffset, m_volumBase + volumOffset));
		base.audio.pitch = Mathf.Clamp(Random.Range(m_pitchBase / (1f + pitchOffset), m_pitchBase * (1f + pitchOffset)), 0.01f, 3f);
		if (loopMode == LoopMode.Default)
		{
			if (isSfx)
			{
				TAudioManager.instance.PlaySound(base.audio, audioClip, base.audio.loop, cutoff);
			}
			else
			{
				TAudioManager.instance.PlayMusic(base.audio, audioClip, base.audio.loop, cutoff);
			}
			if (randomStartPosition)
			{
				base.audio.time = Random.Range(0f, audioClip.length);
			}
			if (!base.audio.loop)
			{
				StopAllCoroutines();
				StartCoroutine(PlayOver(audioClip.length / base.audio.pitch));
			}
		...
		else if (loopMode == LoopMode.MultiLoop)
		{
			if (isSfx)
			{
				TAudioManager.instance.PlaySound(base.audio, audioClip, true, true);
			}
			else
			{
				TAudioManager.instance.PlayMusic(base.audio, audioClip, true, true);
			}
			...
			StopAllCoroutines();
			StartCoroutine(TriggerDelay(audioClip.length / base.audio.pitch));
			SendTriggerEvent(audioClip);
		}
		else if (loopMode == LoopMode.SingleLoop)
		{
			if (isSfx)
			{
				TAudioManager.instance.PlaySound(base.audio, audioClip, true);
			}
			else
			{
				TAudioManager.instance.PlayMusic(base.audio, audioClip, true);
			}
			if (!m_isPlaying && randomStartPosition)
			{
				base.audio.time = Random.Range(0f, audioClip.length);
	...
	public override void Stop()
	{
		...
		else if (loopMode == LoopMode.Default && !base.audio.loop)
		{
			StopAllCoroutines();
		}
		if (isSfx)
		...
		{
			TAudioManager.instance.StopMusic(base.audio);
		}
	}
}
