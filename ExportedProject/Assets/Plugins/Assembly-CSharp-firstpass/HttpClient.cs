...
public class HttpClient
{
	...
	private class HttpTask
	{
		...
		public HttpTask(int taskId, string server, string url, float timeout, string key, string action, string request, string gameObjectName, string componentName, string functionName, object param)
		{
			m_taskId = taskId;
			m_server = server;
			m_url = url;
		...
		public void Start()
		{
			try
			{
				m_www = new WWW(string.Format("{0}?action={1}&random={2}", m_url, m_action, Random.Range(1, 999999)), m_requestData);
			}
			catch
			{
				Complete(-1);
	...
	{
		m_serverInfoMap.Remove(server);
	}

	public int SendRequest(string server, string action, string data, string gameObjectName, string componentName, string functionName, object param)
	{
		if (!m_serverInfoMap.ContainsKey(server))
		{
			Debug.LogError("No Inited Server : " + server);
		}
